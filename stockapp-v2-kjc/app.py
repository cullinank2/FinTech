    ...
    if raw_df is None:
        ...
        if date_col is None:
            ...
            if len(features) < 3:
                ...
                st.markdown("---")
                st.markdown("### 1 · Structural Stability Signal")
                st.caption(
                    "Measures how stable the underlying factor structure is across regimes. "
                    "Higher values indicate structural drift or regime transition."
                )

                st.info(
                    "This signal identifies whether the market is operating under a consistent "
                    "factor regime or undergoing structural change. Elevated readings suggest "
                    "factor relationships are rewiring — a key precursor to regime-driven risk."
                )
                ...
                st.info(
                    "This signal helps identify whether market structure is broadening or compressing. "
                    "Higher readings imply more names are coalescing into the same structural posture, "
                    "increasing hidden factor concentration and regime fragility."
                )
