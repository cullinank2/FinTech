...
def render_universe_period_comparison():
    ...
    st.markdown(
        "This section provides a period comparison for the universe-level analysis."
    )

    period_scores = st.session_state.get("period_scores")
    migration_wide = st.session_state.get("migration_wide")

    if period_scores is None and migration_wide is None:
        st.info("Period comparison data is not available yet.")
        return
    ...
    try:
        migration_fig = create_migration_sankey(migration_wide)
