...
_CROWDING_FLAG  = 50.0   # Appendix B anchor: flag at >50 (flags Disinflation at 67.9)
...
def _render_peer_path_panel(kg) -> None:
    """
    Panel 1: Structural peer lookup via KG cluster proximity.
    Calls kg.get_peers(ticker, regime) — cross-GICS results are the point.
    """
    ...
    with col_left:
        ...
        regime_sel = st.selectbox(
            "Regime",
            options=_REGIME_ORDER,
            index=2,   # default: Disinflation (most recent)
            key="si_peer_regime",
        )
    ...
    with col_right:
        ...
        if not peers:
            st.warning(
                f"No structural peers found for **{ticker_input}** in "
                f"**{regime_sel}**. Confirm the ticker existed in the pipeline "
                "universe and that equity nodes are populated."
            )
            return
        ...
        st.caption(
            "*Tier 1 governance output: deterministic KG path traversal — "
            "stock → cluster_membership edge → cluster node → peer stock nodes. "
...
def _render_crowding_chain_panel(kg) -> None:
    """
    Panel 2: Crowding score trajectory across all three regimes
    with Procrustes disparity as the connecting tissue between them.
    """
    ...
    # ── Pull live data if KG available, else Appendix B fallback ──────────
    crowding_live = {}
    procrustes_live = {}
    using_fallback = True
    ...
    if kg is not None:
        try:
            ...
            for (r_a, r_b), _ in _APPENDIX_B_PROCRUSTES.items():
    ...
    def _crowding_score(regime: str) -> float:
        if not using_fallback and regime in crowding_live:
            return crowding_live[regime].get("crowding_score",
                   _APPENDIX_B_CROWDING[regime])
        return _APPENDIX_B_CROWDING[regime]

    def _procrustes_score(r_a: str, r_b: str) -> tuple:
        key = (r_a, r_b)
        ab  = _APPENDIX_B_PROCRUSTES[key]
        if not using_fallback and key in procrustes_live:
            d = procrustes_live[key].get("disparity",   ab["disparity"])
            n = procrustes_live[key].get("n_tickers",   ab["n_tickers"])
        else:
            d, n = ab["disparity"], ab["n_tickers"]
        return d, n

    pc_score  = [_crowding_score(r) for r in _REGIME_ORDER]
    d01, n01  = _procrustes_score("Post-COVID",  "Rate Shock")
    d02, n02  = _procrustes_score("Post-COVID",  "Disinflation")
    d12, n12  = _procrustes_score("Rate Shock",  "Disinflation")

    if using_fallback:
        st.info("📋 Displaying Appendix B empirical values. Run Period Comparison "
                "to populate live KG data.")

    # ── Metric row: three regime crowding scores ───────────────────────────
    col1, col2, col3 = st.columns(3)

    for col, regime, score, prev_score in zip(
        [col1, col2, col3],
        _REGIME_ORDER,
        pc_score,
        [None] + pc_score[:-1],
        ...
        if prev_score is not None:
            delta_str = f"{score - prev_score:+.1f} vs prior regime"
        flag = " 🚨" if score > _CROWDING_FLAG else ""
        col.metric(
            label=f"{regime} Crowding{flag}",
            value=f"{score:.1f}",
            delta=delta_str,
        )
    ...
    chain_cols[3].markdown(
        f"→\n\n**{d12:.3f}**\n\n*{n12:,} tickers*\n\n"
        f"{'🔴 Major break' if d12 >= 0.30 else '🟢 Stable'}"
    )
    chain_cols[4].markdown(
    ...
    # ── Non-adjacent Procrustes (Post-COVID → Disinflation) ───────────────
    st.markdown(
        f"**Non-adjacent structural distance** "
        f"(Post-COVID → Disinflation): **{d02:.3f}** "
    ...
    st.caption(
        ...
        "across regimes. Threshold ≥ 0.30 = meaningful structural break. "
        "Source: Appendix B empirical data.*"
    )
...
def _render_early_warning_panel(kg) -> None:
    """
    Panel 3: Prospective structural risk flags.
    Crowding >50 triggers flag. Procrustes ≥0.30 triggers break warning.
    Appendix B anchors are the reference values.
    """
    ...
    for regime in _REGIME_ORDER:
        ...
        if flagged:
            status_icon  = "🚨"
            status_label = "ELEVATED — prospective risk signal active"
            bar_color    = "#d62728"
        elif score > 35:
            status_icon  = "🟡"
        ...
        st.markdown(
            f"**{regime}** {status_icon} &nbsp; Score: **{score:.1f}** / 100 &nbsp;|&nbsp; {status_label}"
        )
        st.markdown(
            f"<div style='background:#333;border-radius:4px;height:12px;'>"
            f"<div style='background:{bar_color};width:{bar_pct}%;height:12px;"
    ...
    for label, r_a, r_b in breaks:
        ...
        if (r_a, r_b) in procrustes_live:
            disp = procrustes_live[(r_a, r_b)].get("disparity",  ab["disparity"])
            n    = procrustes_live[(r_a, r_b)].get("n_tickers",  ab["n_tickers"])
        else:
            disp, n = ab["disparity"], ab["n_tickers"]
    ...
    _, d_pc_rs = _APPENDIX_B_PROCRUSTES[("Post-COVID", "Rate Shock")]["disparity"], \
                 _APPENDIX_B_PROCRUSTES[("Post-COVID", "Rate Shock")]["n_tickers"]
    d_pc_rs_val = procrustes_live.get(("Post-COVID","Rate Shock"), {}).get(
                  "disparity", _APPENDIX_B_PROCRUSTES[("Post-COVID","Rate Shock")]["disparity"])
    d_rs_dis_val = procrustes_live.get(("Rate Shock","Disinflation"), {}).get(
                   "disparity", _APPENDIX_B_PROCRUSTES[("Rate Shock","Disinflation")]["disparity"])
    ...
    st.info(
        ...
        f"in Disinflation indicates that equities are piling into an increasingly compressed "
        f"factor space *within a stable structural regime* — the conditions historically "
...
def _render_reasoning_chain_panel(kg) -> None:
    """
    Panel 4: Exposes the deterministic KG traversal logic for a selected regime.
    Calls kg.serialize_subgraph() to surface the graph path as a structured trace.
    This is the governance artifact display panel.
    """
    st.markdown("### 🔍 Reasoning Chain Viewer")
    st.caption(
        "Deterministic KG path traversal for the selected regime — "
        "the complete structural characterization with node → edge → node provenance. "
        "Tier 1 governance artifact: no language model involved."
    )
    ...
    # ── Build node ID list for this regime ────────────────────────────────
    regime_short = regime_sel.replace("-", "").replace(" ", "_").lower()
    ...
    # Add Procrustes transition nodes
    idx = _REGIME_ORDER.index(regime_sel)
    if idx > 0:
        prior = _REGIME_ORDER[idx - 1]
        node_ids.append(f"procrustes_transition:{prior}:{regime_sel}")
    if idx < len(_REGIME_ORDER) - 1:
        nxt = _REGIME_ORDER[idx + 1]
        node_ids.append(f"procrustes_transition:{regime_sel}:{nxt}")

    # ── Pull subgraph from KG ──────────────────────────────────────────────
    subgraph_data = None

    if kg is not None:
        try:
            # Filter to node_ids that actually exist in the graph
    ...
    # ── Render: live subgraph or Appendix B fallback chain ────────────────
    if subgraph_data and subgraph_data.get("nodes"):
        ...
        # Appendix B fallback — deterministic chain from hardcoded values
        ...
        if idx > 0:
            ...
            chain_lines += [
                ...
                f"disparity {ab['disparity']:.3f}, "
                f"{ab['n_tickers']:,} common tickers, "
                f"{'🔴 major structural break' if major else '🟢 structurally stable transition'}",
                "",
            ]

        mig_key = list(_APPENDIX_B_MIGRATION.keys())
        if idx < len(mig_key):
            mig = list(_APPENDIX_B_MIGRATION.values())[idx]
            chain_lines += [
                f"**Edge:** `has_migration_event` →",
                f"**Node:** migration context — "
    ...
    st.markdown("---")
    st.caption(
        "*This panel exposes the deterministic KG traversal that underlies every "
        "Tier 1 governance output. The reasoning chain is the audit trail: "
        "template name + node/edge path + empirical values = structural characterization. "
        "No probabilistic generation. No language model.*"
    )
...
def render_structural_intelligence_tab() -> None:
    ...
    Entry point called from app.py with tab_si context.
    """
    ...
    # ── Retrieve KG instance from session state ────────────────────────────
    import streamlit as _st  # already imported at top of file; alias for clarity
    kg = _st.session_state.get("kg_instance", None)
